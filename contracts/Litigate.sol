...
abstract contract Litigate is Data, Create, Interact, Slasher, EIP712 {
  ...
  function litigateContent(
    ...
    require(
      block.timestamp > content.timestamp + TIME_THRESHOLD,
    ...
    require(
      keccak256(bytes(content.url)) != keccak256(bytes(firstUrl)),
      'Initial content is not litigable'
    );
    ...
    if (contentIndex != 0) {
      ...
      bool isTitleCorrect = keccak256(bytes(content.title)) ==
        keccak256(bytes(existingContent.title));
      bool isSubmittedByCorrect = content.submittedBy ==
        existingContent.submittedBy;
      bool isTagIdsCorrect = content.tagIds.length ==
        existingContent.tagIds.length;
  ...
  function litigateTag(
    ...
    require(
      block.timestamp > tag.timestamp + TIME_THRESHOLD,
      'Time threshold has not passed yet'
    );
    ...
    require(
      keccak256(bytes(tag.name)) != keccak256(bytes(firstTag)),
      'Initial tag is not litigable'
    );
    ...
    if (tagIndex != 0) {
      ...
      bool isNameCorrect = keccak256(bytes(tag.name)) ==
        keccak256(bytes(existingTag.name));
    ...
    _createTagIfNotExists(tag.name, tag.createdBy);
    slashBackend(msg.sender);
    return true;
  }
